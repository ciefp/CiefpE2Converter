#!/bin/bash
##setup command=wget -q "--no-check-certificate" https://raw.githubusercontent.com/ciefp/CiefpE2Converter/main/installer.sh -O - | /bin/sh
...
TMPPATH=/tmp/CiefpE2Converter

if [ ! -d /usr/lib64 ]; then
	PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/CiefpE2Converter
else
	PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/CiefpE2Converter
fi
...
if [ -f /var/lib/dpkg/status ]; then
   ...
   wget https://github.com/ciefp/CiefpE2Converter/archive/refs/heads/main.tar.gz
   tar -xzf main.tar.gz
   cp -r 'CiefpE2Converter-main/usr' '/'
set +e
cd
sleep 2
